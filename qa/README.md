...
Validação local em 9/9/2026, com pontos exclusivamente virtuais.
...
- `browser-audit.json`: persistência após recriar a aplicação, páginas/resoluções/temas, medidas de overflow, labels, cores de selects e respostas de rede.
...
`browser-audit.cjs` requer Playwright disponível no `NODE_PATH` e Google Chrome instalado (ou `CHROME_PATH`). Use `ARENA_QA_URL` para mudar a origem; o padrão é `http://localhost:5174`. O script entra pelo acesso demo, lê a aplicação real, navega e muda o tema pela interface. Não grava tokens. Ele valida os IDs e saldos da base isolada desta rodada, portanto não deve ser apresentado como um teste que prepara automaticamente uma base vazia.
